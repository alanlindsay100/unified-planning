...
#
"""This module defines the action inheritance compiler class."""
...
class ActionInheritanceCompiler(engines.engine.Engine, CompilerMixin):
    """
    ...
    def __init__(self):
        ...
        CompilerMixin.__init__(self, CompilationKind.ACTION_INHERITANCE_COMPILER)
    ...
    def supported_kind() -> ProblemKind:
        ...
        supported_kind.set_action_inheritance("ACTION_INHERITANCE")
        return supported_kind

    @staticmethod
    def supports(problem_kind):
        return problem_kind <= ActionInheritanceCompiler.supported_kind()

    @staticmethod
    def supports_compilation(compilation_kind: CompilationKind) -> bool:
        return compilation_kind == CompilationKind.ACTION_INHERITANCE_COMPILER
    ...
    def resulting_problem_kind(
        ...
        if new_kind.has_action_inheritance():
            new_kind.unset_action_inheritance("ACTION_INHERITANCE")
            new_kind.set_action_inheritance("INDEPENDENT_ACTION_DECLARATIONS")
        return new_kind
    ...
    def _compile(
        ...
        field does not have action inheritance.

        :param problem: The instance of the :class:`~unified_planning.model.Problem` that must be returned without action inheritance.
        :param compilation_kind: The :class:`~unified_planning.engines.CompilationKind` that must be applied on the given problem;
            only :class:`~unified_planning.engines.CompilationKind.ACTION_INHERITANCE_COMPILER` is supported by this compiler
        ...
        new_problem._actions = list(map(lambda a: a.compile_action(), new_problem._actions))
        
        """Problem(f"{problem.name}_{self.name}", env)
